...
class BFSImprovedPlayer(wws.OfflinePlayer):
        ...
        while not frontier.empty() and (not reached_gold):
            ...
            for action in all_actions:     #Looping through Left, right, shoot and move and adding resulting states to frontier. Grab and climb are handled once the gold is reached
                ...
                if action == left:
                    ...
                    # It makes no sense to turn right and then immediatly turn left
                    if not len(parent_state[4]) == 0 and parent_state[4][-1] == right:
                        logical_move = False
                    ...
                    # It makes no sense to turn left and then immediatly turn right
                    if not len(parent_state[4]) == 0 and parent_state[4][-1] == left:
                        logical_move = False
                    ...
                    if not wumpus_dead:     #It only makes sense to shoot if the wumpus is in sight
                        logical_move = False
                ...
                if legal_move and logical_move:
...
def fixed_offline(world_json: str = WUMPUS_WORLD):
    ...
    # Run a player with knowledge about the world
    wws.run_episode(world, BFSImprovedPlayer())
