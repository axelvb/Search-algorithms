...
class AStarPlayer(wws.OfflinePlayer):
    ...
    def manhattan_distance(self, world_info:dict): 
        # Using manhattan-distance as heuristic
        xDif = abs(world_info['Hunter'][0][0] - world_info['Gold'][0][0])  
        yDif = abs(world_info['Hunter'][0][1] - world_info['Gold'][0][1])  
        
        return xDif + yDif
    

    def Euclidean(self, pos: tuple, world_info:dict): 
        # Using Euclidean as heuristic
        xDif = abs(world_info['Hunter'][0][0] - world_info['Gold'][0][0])  
        yDif = abs(world_info['Hunter'][0][1] - world_info['Gold'][0][1])
        
        return (xDif**2 + yDif**2)**(1/2)
    ...
    def start_episode(self, world: wws.WumpusWorld) -> Iterable[wws.Hunter.Actions]:
        ...
        #Structure of states: [Arrow used, Wumpus dead, direction, player pos, actions, cost taken to get to state]
        start_state = [False, False, 'N', (0,0), [], 0]

        frontier.put(PrioritizedItem(0, start_state))   #Priority of actions taken

        reached = {(0,0,'N') : start_state} # A decent way to store the different states. Each square (the key) can be reached in a different state (the list)
        solution = [] 
        # When the gold is reached the best journey back will always be the way you came, so no need to compute that as well
        
        while not frontier.empty() and (not bool(solution) or solution[5] >frontier.queue[0].item[5]):
            ...
            for action in all_actions[:4]:     #Looping through Left, right, shoot and move and adding resulting states to frontier or/and reached. Grab and climb are handled once the gold is reached
                parent_cost = parent_state[5]
                legal_move = True
                if action == left:
                    ...
                    parent_cost += 1 #cost of rotating left
                    new_state = [parent_state[0], parent_state[1], new_direction, parent_state[3], actions, parent_cost]
                    ...
                    if parent_state[2] == 'E':
                        ...
                        new_direction = 'W'
                    parent_cost += 1 #cost of rotating right
                    new_state = [parent_state[0], parent_state[1], new_direction, parent_state[3], actions, parent_cost]
                    ...
                    if new_pos in world_info['Pits'] or (new_pos in world_info['Wumpus'] and not parent_state[1]):
                        legal_move = False
                        parent_cost += 1000 #cost of going into pit/being eaten by wumpus
                    ...
                    if legal_move:
                        ...
                        parent_cost += 1 #cost of moving in given direction
                        new_state = [parent_state[0], parent_state[1], parent_state[2], new_pos, actions, parent_cost]
                    ...
                    parent_cost += 10 #cost of shooting
                    new_state = [arrow_used, wumpus_dead, current_dir, current_pos, actions, parent_cost] 
                    
               
                    
                # Adding the new state to the children list
                if legal_move:
                    
                    children[action] = new_state

            #Checking if the state is reached with fewer or same amount of moves
            for key in children:
                s = children[key]
                spos = (s[3][0], s[3][1], s[2])
                if (spos in reached and s[1] and not reached[spos][1] ) :
                    frontier.put(PrioritizedItem(parent_cost + self.manhattan_distance(world_info),children[key]))
                elif (spos not in reached) or (parent_cost < reached[spos][5]):
                    reached[spos] = children[key]
                    frontier.put(PrioritizedItem(parent_cost + self.manhattan_distance(world_info),children[key]))
                        
                if (s[3] in world_info['Gold']) and (len(solution)==0 or solution[5] > parent_cost):
                    solution = s


        #Now the gold is reached and the state is saved
        #We have to find the way back as well
        actions_to_gold = solution[4]
...
def classic_offline(size: int = 0):
    ...
    wws.run_episode(world, AStarPlayer())
