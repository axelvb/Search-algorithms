...
class UCSPlayer(wws.OfflinePlayer):
    

    def findNeighbours(self, pos: tuple, world_info:dict):
            #Function that finds the neighbours of a tuple
    ...
    def start_episode(self, world: wws.WumpusWorld) -> Iterable[wws.Hunter.Actions]:
        ...
        #Saving all actions to use them for comparison 
        move = all_actions[0]
        right = all_actions[1]
        left = all_actions[2]
        shoot = all_actions[3]
        ...
        reached = {(0,0,'N') : start_state} # A decent way to store the different states. Each square with direction (the key) can be reached in a different state (the list)
        solution = [] 
        ...
        while not frontier.empty() and (not bool(solution) or solution[5] >frontier.queue[0].item[5]):
            ...
            for action in all_actions[:4]:     #Looping through Left, right, shoot and move and adding resulting states to frontier. Grab and climb are handled once the gold is reached
            ...
            for key in children:
                ...
                if (spos in reached and s[1] and not reached[spos][1] ) :
                    frontier.put(PrioritizedItem(parent_cost, children[key]))
...
WUMPUS_WORLD = '''
    {
        ...
        "size": [7, 7],
        "hunters": [[0, 0]],
        "pits": [[4, 0], [3, 1], [2, 2], [6, 2], [4, 4], [3, 5], [4, 6], [5, 6], [0, 2]],
        "wumpuses": [[1, 2]],
        "exits": [[0, 0]],
        "golds": [[6, 3]],
        "blocks": []
...
def fixed_offline(world_json: str = WUMPUS_WORLD):
    ...
    # Run a player with knowledge about the world
    wws.run_episode(world, UCSPlayer())
